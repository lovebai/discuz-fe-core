...
export const input = (vditor: IVditor, range: Range, event?: InputEvent | InsertValueEvent, insertValue?: string) => {
    ...
    if (event && event.inputType !== "formatItalic"
      ...
      // 解决空格导致的markdown渲染问题
      // html = html
      //   .replace(/(<p data-block="0">)(\u200b*)(\s*)(.*)(<wbr>)/, (p1, p2, p3, p4, p5, p6) => {
      //     return p2 + p3.replace(/\u200b/g, '&nbsp;') + p4 + p5 + p6;
      //   })
      //   .replace(/(<p data-block="0">)(<wbr>)(<br>)/, (p1, p2, p3, p4) => {
      //     return p2 + '&nbsp;' + p3 + p4;
      //   });
        ...
        // 设置光标
        setRangeByWbr(vditor.wysiwyg.element, range, event, insertValue);
