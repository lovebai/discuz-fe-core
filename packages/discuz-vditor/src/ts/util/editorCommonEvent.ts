...
export const focusEvent = (vditor: IVditor, editorElement: HTMLElement) => {
  editorElement.addEventListener("focus", (event) => {
        if (vditor.options.focus) {
            vditor.options.focus(getMarkdown(vditor), event);
        }
...
export const selectEvent = (vditor: IVditor, editorElement: HTMLElement) => {
  function bubble() {
    ...
    clearTimeout(vditor.bubbleToolbar.bubbleToolbarTimeoutId);
    vditor.bubbleToolbar.bubbleToolbarTimeoutId = window.setTimeout(() => {
      if (selectText && !vditor.options.toolbarConfig.bubbleHide) {
        vditor.bubbleToolbar.show(vditor);
      } else {
  ...
  // https://developer.mozilla.org/zh-CN/docs/Web/API/Document/selectionchange_event
