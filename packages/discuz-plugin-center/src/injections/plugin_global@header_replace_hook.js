...
export default function plugin_global2header_replace_hook(pluginData) {
    return {
        render: (props) => {
            // 只会提供属于插件自身的数据
            // 必须严格控制可传入到插件的数据

            const { renderData, pluginStore, pluginAction, site, userInfo, isLogin, message, onSearch } = props;
